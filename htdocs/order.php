...
while($row_cart = mysqli_fetch_array($run_cart)){
    ...
    while($row_products = mysqli_fetch_array($run_products)){
        ...
        $insert_customer_order = "insert into customer_orders (order_id,customer_id,due_amount,invoice_no,qty,order_date,order_status) values ('0','$customer_id','$sub_total','$invoice_no','$pro_qty',NOW(),'$status')";
        
        $run_customer_order = mysqli_query($db,$insert_customer_order);
        
        $insert_pending_order = "insert into pending_order (cus_id,invoice_id,product_id,qty,order_statuss) values ('$customer_id','$invoice_no','$pro_id','$pro_qty','$status')";
